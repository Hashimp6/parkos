...
// If real data is provided, use it as-is (missing fields just won't render).
// Only fall back to dummy when NO data is passed at all.
const merge = (r, f) => {
  if (!r || Object.keys(r).length === 0) return f;
  return r;
};
...
export default function Company5({ data: rawData }) {
  ...
  const addrStr = [addr.building, addr.street, addr.city, addr.state, addr.pincode]
    .filter(Boolean).join(", ");

  // ── Only include nav sections that have data ──────────────────────────────
  const navLinks = [
    has(D.about)    && { k: "about",    l: "About"   },
  ...
  const socials = [
    ...
    has(D.contacts?.youtube)   && { icon: Icon.yt, href: D.contacts.youtube,                        t: "YouTube"   },
    has(D.contacts?.whatsapp)  && { icon: Icon.wa, href: `https://wa.me/${D.contacts.whatsapp.replace(/\D/g,"")}`, t: "WhatsApp" },
  ].filter(Boolean);

  // ── Stats bar — only show stats where data exists ─────────────────────────
  const statsItems = [
    has(D.foundedYear) && !isNaN(D.foundedYear) && { n: `${YR - D.foundedYear}+`, l: "Years Experience" },
    has(D.members)     && { n: `${D.members.length}+`,  l: "Team Members"      },
    has(D.clients)     && { n: `${D.clients.length}+`,  l: "Clients Served"    },
    has(D.projects)    && { n: `${D.projects.length}+`, l: "Projects Delivered" },
  ].filter(Boolean);
  ...
  const eyebrow = (txt, light = false) => (
    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
      ...
      <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: light ? G.light : G.base, fontFamily: "'Outfit', sans-serif" }}>
        {txt}
      </span>
    </div>
  );
    ...
    <div style={{ fontFamily: "'Outfit', sans-serif", background: W.off, color: G.deepest, overflowX: "hidden" }}>
      ...
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .social-btn:hover { background: ${G.base}; color: #fff; border-color: ${G.base}; }
        ...
        .form-input:focus { border-color: rgba(116,198,157,0.55); }
        ...
        .form-textarea:focus { border-color: rgba(116,198,157,0.55); }
        .form-textarea::placeholder { color: rgba(255,255,255,0.3); }

        /* ── Client grid cell hover ── */
        .client-cell { background: #fff; transition: background 0.2s; }
        .client-cell:hover { background: ${G.ghost}; }
      ...
      <nav style={{
        ...
        <div style={{ ...maxW, padding: "0 32px", width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, cursor: "pointer", flexShrink: 0 }}
            ...
            {has(D.logo) ? (
              <img src={D.logo} alt={`${D.companyName} logo`} style={{ height: 36, objectFit: "contain" }} />
            ) : (
              <div style={{ width: 38, height: 38, borderRadius: 9, background: G.base, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 800, color: "#fff", fontFamily: "'Syne', sans-serif", flexShrink: 0 }}>
                {ini(D.companyName)}
              </div>
            )}
            <span style={{ fontSize: 16, fontWeight: 700, color: "#fff", fontFamily: "'Syne', sans-serif", letterSpacing: "-0.01em", whiteSpace: "nowrap" }}>
              {D.companyName}
            </span>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            {has(D.contacts?.email) && (
              <a href={`mailto:${D.contacts.email}`} className="primary-btn hide-mob"
                style={{ padding: "9px 20px", fontSize: 13 }}>
                Get In Touch
              </a>
            )}
            {/* Hamburger — always in DOM, hidden via inline style on desktop */}
            <button
              ...
              style={{
                ...
                color: "#fff", display: "none", alignItems: "center",
        ...
        {menuOpen && (
          <div style={{
            ...
            {navLinks.map(({ k, l }) => (
              <div key={k} className="mob-nav-item" onClick={() => go(k)}>{l}</div>
            ))}
            {has(D.contacts?.email) && (
              <a href={`mailto:${D.contacts.email}`} className="primary-btn"
      ...
      <section id="sec-hero" style={{
        ...
        background: has(D.banner)
          ? `linear-gradient(160deg, rgba(11,31,20,0.92) 0%, rgba(18,43,28,0.85) 55%, rgba(30,77,48,0.80) 100%), url(${D.banner}) center/cover no-repeat`
          : `linear-gradient(160deg, ${G.deepest} 0%, ${G.dark} 55%, ${G.rich} 100%)`,
        display: "flex", flexDirection: "column", justifyContent: "center",
        position: "relative", overflow: "hidden",
      }}>
        {/* Decorative rings */}
        <svg style={{ position: "absolute", right: "-3%", top: "50%", transform: "translateY(-50%)", opacity: 0.06, pointerEvents: "none" }}
          width="520" height="520" viewBox="0 0 520 520" aria-hidden="true">
          {[60,120,180,240,300,360].map(r => (
            <circle key={r} cx="520" cy="260" r={r} fill="none" stroke={G.light} strokeWidth="1" />
          ))}
        </svg>
          ...
          <div style={{ maxWidth: 760 }}>
            ...
            {(has(D.businessPark) || has(addr.city)) && (
              <div className="fu" style={{ display: "inline-flex", alignItems: "center", gap: 8, marginBottom: 28, padding: "6px 14px 6px 8px", background: "rgba(116,198,157,0.12)", border: "1px solid rgba(116,198,157,0.25)", borderRadius: 100 }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 700, color: G.light, letterSpacing: "0.12em", textTransform: "uppercase", fontFamily: "'Outfit', sans-serif" }}>
                  {[D.businessPark !== "Other" ? D.businessPark : null, addr.city].filter(Boolean).join(" · ")}
                </span>
              </div>
            )}
            ...
            {has(D.tagline) && (
              <h1 className="fu1" style={{
                margin: "0 0 22px",
                fontSize: "clamp(36px,5.5vw,76px)", fontWeight: 800,
            ...
            <div className="fu3 hero-cta" style={{ display: "flex", flexWrap: "wrap", gap: 14, marginBottom: has(D.tags) ? 48 : 0 }}>
              {has(D.services) && (
        ...
        <div style={{ display: "flex", justifyContent: "center", paddingBottom: 36, position: "relative", zIndex: 1 }}>
          <button onClick={() => go("about")} aria-label="Scroll down"
            style={{ background: "none", border: "none", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 7, opacity: 0.4, transition: "opacity 0.2s", padding: 0 }}
            ...
            <svg width="14" height="22" viewBox="0 0 14 22" fill="none">
              ...
              <circle className="scroll-dot" cx="7" cy="7" r="2" fill="white" />
            </svg>
          </button>
        </div>
      </section>

      {/* ── Stats bar — only rendered if any stat exists ── */}
      {statsItems.length > 0 && (
        <div className="stats-bar" style={{ background: G.base, display: "grid", gridTemplateColumns: `repeat(${statsItems.length}, 1fr)` }}>
          {statsItems.map(({ n, l }, i) => (
            <div key={i} style={{ padding: "36px 20px", textAlign: "center", borderRight: i < statsItems.length - 1 ? "1px solid rgba(255,255,255,0.18)" : "none" }}>
      ...
      ══════════════════════════════════════════ */}
      {has(D.about) && (
        <section id="sec-about" className="section-pad" style={{ ...sectionPad, background: W.off }}>
          <div className="two-col" style={{ ...maxW, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "center" }}>
            ...
            <div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
                {has(D.website) && (
                  <a href={D.website} target="_blank" rel="noreferrer" className="primary-btn">
                    Visit Website {Icon.ext}
                  </a>
                )}
      ...
      ══════════════════════════════════════════ */}
      {has(D.services) && (
        <section id="sec-services" className="section-pad" style={{ ...sectionPad, background: G.dark }}>
          <div style={maxW}>
            ...
            <div className="three-col" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 20 }}>
              {D.services.map((s, i) => (
      ...
      ══════════════════════════════════════════ */}
      {has(D.projects) && (
        <section id="sec-projects" className="section-pad" style={{ ...sectionPad, background: W.off }}>
          <div style={maxW}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 22 }}>
              {D.projects.map((p, i) => {
                const bgs = [
                  `linear-gradient(135deg,${G.deepest} 0%,${G.rich} 100%)`,
                  `linear-gradient(135deg,${G.dark} 0%,${G.base} 100%)`,
                  `linear-gradient(135deg,#0a1a0f 0%,${G.mid} 100%)`,
                  `linear-gradient(135deg,${G.rich} 0%,${G.muted} 100%)`,
                ];
                return (
                  <div key={i} className="card" style={{ overflow: "hidden" }}>
                    ...
                    <div style={{ padding: "24px" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                        ...
                        {has(p.link) && (
                          <a href={p.link} target="_blank" rel="noreferrer" style={{ color: G.base, flexShrink: 0, marginLeft: 8 }} aria-label={`View ${p.name}`}>
                            {Icon.ext}
                          </a>
                        )}
                      </div>
      ...
      ══════════════════════════════════════════ */}
      {has(D.members) && (
        <section id="sec-team" className="section-pad" style={{ ...sectionPad, background: G.deepest }}>
          <div style={maxW}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: 20 }}>
              {D.members.map((m, i) => (
                <div key={i} className="dark-card"
                  style={{ padding: "30px 18px", textAlign: "center", cursor: has(m.url) ? "pointer" : "default" }}
                  onClick={() => has(m.url) && window.open(m.url, "_blank")}
                  role={has(m.url) ? "link" : undefined}
                  tabIndex={has(m.url) ? 0 : undefined}>
                  {has(m.image) ? (
                    <img src={m.image} alt={m.name}
                      style={{ width: 76, height: 76, borderRadius: "50%", objectFit: "cover", margin: "0 auto 14px", display: "block", border: `3px solid ${G.rich}` }} />
                  ) : (
                    <div style={{ width: 76, height: 76, borderRadius: "50%", background: avc(m.name), border: "3px solid rgba(116,198,157,0.25)", margin: "0 auto 14px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, fontWeight: 700, color: G.light, fontFamily: "'Syne', sans-serif" }}>
                      {ini(m.name)}
                    </div>
                  )}
                  ...
                  {has(m.url) && (
                    <p style={{ margin: "10px 0 0", fontSize: 12, color: G.light, display: "flex", alignItems: "center", justifyContent: "center", gap: 4 }}>
                      Profile {Icon.ext}
                    </p>
                  )}
                </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))" }}>
              {D.clients.map((c, i) => (
                <div key={i} className="client-cell"
                  ...
                  style={{ padding: "28px 18px", textAlign: "center", border: `1px solid ${G.pale}`, display: "flex", flexDirection: "column", alignItems: "center", gap: 12, cursor: has(c.website) ? "pointer" : "default" }}>
                  {has(c.logo) ? (
                    <img src={c.logo} alt={c.name}
                      style={{ height: 36, objectFit: "contain", filter: "grayscale(1)", opacity: 0.5, transition: "all 0.2s" }}
              ...
              {D.gallery.map((g, i) => {
                ...
                return has(g.imageUrl) ? (
                  <div key={i} style={{ borderRadius: 12, overflow: "hidden", position: "relative" }}>
                    <img src={g.imageUrl} alt={g.caption || `Gallery ${i + 1}`}
                      style={{ width: "100%", height: 200, objectFit: "cover", display: "block" }} />
                    {has(g.caption) && (
                      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "10px 14px", background: "linear-gradient(transparent,rgba(0,0,0,0.65))", fontSize: 12, color: "#eee" }}>
                        {g.caption}
                      </div>
                    )}
                  </div>
                ) : (
                  <div key={i} style={{ borderRadius: 12, overflow: "hidden", height: 200, background: bgs[i % bgs.length], display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 10 }}>
                    <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="rgba(116,198,157,0.25)" strokeWidth="1" aria-hidden="true">
                      <rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/>
                    </svg>
                    {has(g.caption) && (
                      <p style={{ margin: 0, fontSize: 12, color: "rgba(116,198,157,0.35)", textAlign: "center", padding: "0 16px" }}>
                        {g.caption}
                      </p>
                    )}
                  </div>
      ...
      {/* ══════════════════════════════════════════
          CONTACT
      ══════════════════════════════════════════ */}
      <section id="sec-contact" className="section-pad" style={{ ...sectionPad, background: G.dark }}>
        <div className="two-col" style={{ ...maxW, display: "grid", gridTemplateColumns: "1.1fr 1fr", gap: 80, alignItems: "start" }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20, marginBottom: 40 }}>
              {has(D.contacts?.email) && (
                <a href={`mailto:${D.contacts.email}`} className="contact-link">
              ...
              {has(D.contacts?.phone) && (
                <a href={`tel:${D.contacts.phone}`} className="contact-link">
                  <div style={{ width: 42, height: 42, borderRadius: 9, background: "rgba(116,198,157,0.1)", border: "1px solid rgba(116,198,157,0.2)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    {Icon.phone}
                  </div>
                  {D.contacts.phone}
              ...
              {has(D.website) && (
                <a href={D.website} target="_blank" rel="noreferrer" className="contact-link">
                  <div style={{ width: 42, height: 42, borderRadius: 9, background: "rgba(116,198,157,0.1)", border: "1px solid rgba(116,198,157,0.2)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    {Icon.globe}
                  </div>
              ...
              <div>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                  {socials.map(({ icon, href, t }) => (
                    <a key={t} href={href} target="_blank" rel="noreferrer" title={t} className="social-btn" aria-label={t}>
                      {icon}
                    </a>
                  ))}
                </div>
          ...
          <div style={{ background: G.mid, border: "1px solid rgba(116,198,157,0.15)", borderRadius: 16, padding: "36px 32px" }}>
            {formSent ? (
              <div style={{ textAlign: "center", padding: "32px 0" }}>
                <div style={{ width: 56, height: 56, borderRadius: "50%", background: "rgba(116,198,157,0.15)", border: "1px solid rgba(116,198,157,0.3)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px" }}>
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={G.light} strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>
                </div>
                <p style={{ margin: "0 0 8px", fontSize: 18, fontWeight: 700, color: "#fff", fontFamily: "'Syne', sans-serif" }}>Message Sent!</p>
                <p style={{ margin: "0 0 24px", fontSize: 14, color: W.muted }}>We'll get back to you shortly.</p>
                <button onClick={() => { setFormSent(false); setFormData({ name: "", email: "", phone: "", message: "" }); }}
                  className="outline-btn-white" style={{ fontSize: 13 }}>
                  Send Another
                </button>
              </div>
            ) : (
              <>
                ...
                <input
                  ...
                  placeholder="Phone (optional)" value={formData.phone}
                  onChange={handleFormChange} />
                <textarea
                  className="form-textarea" name="message" rows={4}
                  placeholder="Tell us about your project…" value={formData.message}
                  onChange={handleFormChange} />
                <button
                  onClick={handleFormSubmit}
                  style={{ width: "100%", background: G.base, color: "#fff", border: "none", borderRadius: 8, padding: "14px", fontSize: 15, fontWeight: 600, fontFamily: "'Syne', sans-serif", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, transition: "background 0.2s, opacity 0.2s", opacity: (!formData.name || !formData.email) ? 0.55 : 1 }}
                  onMouseEnter={e => { if (formData.name && formData.email) e.currentTarget.style.background = G.muted; }}
      ...
      <footer style={{ background: G.deepest, padding: "28px 32px", borderTop: "1px solid rgba(116,198,157,0.1)" }}>
        <div style={{ ...maxW, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 14 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            {has(D.logo) ? (
              <img src={D.logo} alt={`${D.companyName} logo`} style={{ height: 24, objectFit: "contain" }} />
            ) : (
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 18, flexWrap: "wrap" }}>
            {socials.slice(0, 5).map(({ icon, href, t }) => (
              <a key={t} href={href} target="_blank" rel="noreferrer" title={t} aria-label={t}
                style={{ color: W.faint, textDecoration: "none", transition: "color 0.2s", display: "flex" }}
            ...
            {has(D.website) && (
              <a href={D.website} target="_blank" rel="noreferrer"
                style={{ fontSize: 12, color: W.faint, textDecoration: "none", transition: "color 0.2s" }}
                onMouseEnter={e => e.currentTarget.style.color = G.light}
                onMouseLeave={e => e.currentTarget.style.color = W.faint}>
                {D.website.replace(/^https?:\/\//, "")}
              </a>
